...
# -------------------------------
def load_model():
    # Asegúrate de tener el modelo en el directorio correcto, puedes usar 'pickle' o 'tensorflow'
...
if uploaded_file is not None:
    ...
    img_resized = image.resize((28, 28))  # Cambiar tamaño de la imagen
    img_array = np.array(img_resized) / 255.0  # Normalizar la imagen
    img_array = np.expand_dims(img_array, axis=(0, -1))  # Añadir dimensión para el batch y canal de color

    # -------------------------------
    # 4. Predicción con el modelo
    # -------------------------------
    prediction = model.predict(img_array)
    pred_class = (prediction > 0.5).astype("int")[0][0]  # 0.5 como umbral de clasificación binaria
